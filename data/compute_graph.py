...
# gets all nodes within a certain radius (SELF CREATED)
def _get_nodes_in_radius(node, nodes, radius):
    nearby_locations = []
    for i in range(0, len(nodes)):
        if _dist(node, nodes[i]) < radius:
            nearby_locations.append(nodes[i])
    return nearby_locations

# gets all edges attached to the nodes within the radius (SELF CREATED)
def _get_edges_in_radius(node, nodes, edges, radius):
    included_edges = []
    for i in range(0, len(edges)):
        nearby = _get_nodes_in_radius(node, nodes, radius)
        for j in range(0, len(nearby)):
            for k in range(0, len(nearby)):
                if edges[i][0] == nearby[j][0] and edges[i][1] == nearby[k][0]:
